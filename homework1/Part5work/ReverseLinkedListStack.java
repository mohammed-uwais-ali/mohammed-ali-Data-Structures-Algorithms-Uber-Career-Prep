...
//	public Node tail; //would need a tail for a palindrome
	...
	public Node reverseALListStack(Node head) {
        
 	   Stack<Node> nodeStacker = new Stack<Node>();
 	   Node current = head;
 	   
 	   while (current != null) {
 		   nodeStacker.push(current);
 		   current = current.next;
 	   }
 	   
 	   head = nodeStacker.pop();
 	   current = head;
 	   
 	   while (!nodeStacker.isEmpty()) {
 		   current.next = nodeStacker.pop();
 		   current = current.next;
 	   }
 	   current.next = null; //ending point
